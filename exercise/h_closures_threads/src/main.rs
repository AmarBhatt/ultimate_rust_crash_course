...
fn main() {
    ...
    }); // NOTE: move is already implicitly implied because we are passing my_vector to the function in the thread, but adding "move" explicitly is good practice
    ...
    // Note instead of a match you could:
    //      let sum = res.unwrap();  but this doesn't handle errors
    ...
    for word in vec!["Hello", "this", "is", "Amar", "Bhatt"] {
        println!("Main thread: {}", word);
        tx.send(word).unwrap();
        tx2.send(word).unwrap();
        pause_ms(200);
    }
